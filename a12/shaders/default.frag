...
void main() {
    ...
    oColor = vec4(ambient + (brdf(lightDir, viewDir, normal) * 100) * light.lightColour.rgb * (max(dot(normal, lightDir), 0.0)), 1.0f) / pow(length(light.lightPos.xyz - v2fPosition), 2);
}
