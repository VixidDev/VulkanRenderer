...
FXAAPipeline::FXAAPipeline(VulkanWindow* window, PipelineLayout* pipelineLayout, RenderPass* renderPass) : Pipeline(window) {
...
void FXAAPipeline::recreate() {
	vk::ShaderModule vert = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/fullScreen.vert.spv");
	vk::ShaderModule frag = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/fxaa.frag.spv");
