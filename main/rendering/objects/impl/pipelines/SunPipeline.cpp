...
SunPipeline::SunPipeline(VulkanWindow* window, PipelineLayout* pipelineLayout, RenderPass* renderPass) : Pipeline(window) {
...
void SunPipeline::recreate() {
	vk::ShaderModule vert = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/fullScreen.vert.spv");
	vk::ShaderModule frag = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/sun.frag.spv");
