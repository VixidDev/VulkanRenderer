...
TonemapPipeline::TonemapPipeline(VulkanWindow* window, PipelineLayout* pipelineLayout, RenderPass* renderPass) : Pipeline(window) {
...
void TonemapPipeline::recreate() {
	vk::ShaderModule vert = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/fullScreen.vert.spv");
	vk::ShaderModule frag = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/tonemap.frag.spv");
