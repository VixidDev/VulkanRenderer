...
void BlurPipeline::recreate() {
	vk::ShaderModule vert = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/fullScreen.vert.spv");
	vk::ShaderModule frag = loadShaderModule(*this->window->getDevice(), "assets/main/shaders/blur.frag.spv");

	this->kernelSize = this->kernelSizePtr ? *this->kernelSizePtr : 0;
