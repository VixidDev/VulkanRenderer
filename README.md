...
- Since we sample normals from normal maps, the normals are in tangent space and require a tangent-bitangent-normal (TBN) frame to transform them from tangent space to world space for lighting calculations. Typically, implementations would pass normals and tangents to the vertex shader as vertex attributes, sometimes in large formats like RGBA16 or possibly RGB8 if high precision is not required. Since TBN frames are essentially rotation matrices they can be represented as a quaternion of 4 values, using the identity $1 = x^2 + y^2 + z^2 + w^2$ we know we can then reconstruct the largest value if we have the other 3. To know which index of the quaternion contains the largest value we only need 2 bits [0-3], this plays itself nicely to use the format `VK_FORMAT_A2R10G10B10_UNORM_PACK32` to store the smallest 3 values in the RGB channels with 10 bits each and store the largest value index in the alpha channel with 2 bits. Packing the quaternion of the TBN frame into this format means we only use 32 bits to pass the partial quaternion to the vertex shader, where we can then unpack it, decode it and turn it back into a matrix to be used when sampling from a normal map, more info about this technique can be found in the [BitSquid Low Level Animation](https://github.com/niklasfrykholm/blog/blob/master/2009/the-bitsquid-low-level-animation-system.md) blog.
