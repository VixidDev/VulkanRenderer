...
InputModel loadCompressedWavefrontObj(const char* path) {
	assert(path);
	
	// Ask rapidobj to load the requested file
	const rapidobj::MaterialLibrary mlib = rapidobj::MaterialLibrary::SearchPath(std::filesystem::absolute(std::filesystem::path(path).remove_filename()));

	ZStdIStream ins(path);
	auto result = rapidobj::ParseStream(ins, mlib);
	if (result.error)
		throw Utils::Error("Unable to load OBJ file '%s': %s", path, result.error.code.message().c_str());
	...
	for (const auto& shape : result.shapes) {
		...
		for (const auto matId : activeMaterials) {
			...
			const auto firstVertex = ret.positions.size();
			
			for (std::size_t i = 0; i < shape.mesh.indices.size(); ++i) {
				const auto faceId = i / 3; // Always triangles; see Triangulate() above
				const auto faceMat = std::size_t(shape.mesh.material_ids[faceId]);
			...
			const auto vertexCount = ret.positions.size() - firstVertex;
